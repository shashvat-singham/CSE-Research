...
def specs():
    ...
    config = {
        'num_rules': 1, #Number of rules
        'size_rules' : 1, #Number of symbols in RHS
...
def find_original_grammar(eps=True):
    ...
    original_grammar = [['T* ','T
T']]
    # return get_original_grammar()
    return reverse_grammar(original_grammar)
    
def get_parse_table(convert = False):
    parse_table = [{'non_term':'T*','$':'0','a':'0','c':'0','b':'0'},{'non_term':'T','$':'0','a':'0','c':'0','b':'0'},{'non_term':'R','$':'0','a':'0','c':'0','b':'0'}]
    return parse_table
